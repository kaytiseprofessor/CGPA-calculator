...
const TermBlock: React.FC<TermBlockProps> = React.memo(({ term, onUpdateTerm, onDeleteTerm, isOnlyTerm }) => {
  ...
  const getGpaColor = (gpa: number) => {
    ...
    if (gpa >= 3.00) return 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/50 dark:text-blue-300 dark:border-blue-800';
    if (gpa >= 2.25) return 'bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/50 dark:text-yellow-300 dark:border-yellow-800';
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all hover:shadow-md animate-fade-in">
      <div className="bg-gray-50/80 dark:bg-gray-700/80 backdrop-blur-sm px-5 py-4 border-b border-gray-200 dark:border-gray-600 flex flex-wrap justify-between items-center gap-3">
        <div className="flex items-center gap-3 flex-grow">
          ...
          <input
            ...
            className="bg-transparent font-bold text-lg text-gray-800 dark:text-white focus:ring-2 focus:ring-emerald-500 rounded px-2 py-1 outline-none border border-transparent hover:border-gray-300 dark:hover:border-gray-500 transition-colors w-full sm:w-auto"
      ...
      <div className="p-4 sm:p-6">
        ...
        <button
          onClick={addCourse}
          className="mt-4 w-full py-2.5 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-500 dark:text-gray-400 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-emerald-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all flex items-center justify-center gap-2"
        >
